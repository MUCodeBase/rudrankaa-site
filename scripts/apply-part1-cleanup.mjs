...
// Keep cache-buster values aligned with the files changed in this cleanup.
...
// Defer the two decorative below-fold watermarks without changing their visual dimensions.
for (const watermark of [
  "assets/watermarks/ardhanarishwar-watermark.webp",
  "assets/watermarks/om-watermark.webp",
]) {
  const oldBlock = `src="${watermark}"\n          alt=""\n          width="600"\n          height="600"\n          decoding="async"`;
  const newBlock = `src="${watermark}"\n          alt=""\n          width="600"\n          height="600"\n          loading="lazy"\n          fetchpriority="low"\n          decoding="async"`;
  await replaceExact("index.html", oldBlock, newBlock);
}

// Use the current legal stylesheet cache key directly instead of loading v2 and rewriting it to v3 later.
await replaceExact(
  "script-v2.js",
...
// Allow one-finger vertical reading at the fit-to-width 100% baseline while
// preserving horizontal drag only for explicitly zoomed content.
await replaceExact(
  "myth-busters.js",
...
// Remove files that no longer participate in the production or preview architecture.
for (const obsoleteFile of [
  ".github/workflows/logo-600-preview.yml",
...
// Remove this one-time migration and its workflow from the final branch state.
await rm(resolve("scripts/apply-part1-cleanup.mjs"), { force: true });
await rm(resolve(".github/workflows/part1-cleanup.yml"), { force: true });

console.log("Part 1 repository cleanup applied successfully.");
